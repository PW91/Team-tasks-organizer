...
import { pasteFirstTask, pasteNextTask } from '../../reduxActions/calendarDataActions';
...
class UserCell extends Component {
    ...
    handleAddTaskPopup( id, e ) {
        ...
        const cellTasksFlag = Boolean( e.target.parentElement.querySelector( '.single-task' ) );
    ...
    handlePaste( cellId, e ) {
        ...
        if ( cellTasksFlag ) {
            store.dispatch( pasteNextTask( this.props.copiedTask, cellId, this.props.weekNum, this.props.year ) );
        } else {
            store.dispatch( pasteFirstTask( this.props.copiedTask, cellId, this.props.weekNum, this.props.year) );
        } 
    }
    ...
    render() {
        ...
        if ( tasksObj ) {
            tasksContainerEl = 
                <TasksContainer tasks = { tasksObj.tasks } copy = { this.props.copyMode } copied = { this.props.copiedTask } />
        }
        ...
        if ( this.props.copyMode ) {
            buttonEl =
                <button onClick = { this.handlePaste.bind( null, this.props.cellId ) }>Wklej</button>
        } else {
            buttonEl = 
                <button onClick = { this.handleAddTaskPopup.bind( null, this.props.cellId ) }>Dodaj</button>
        }
