...
import { deleteTask, deleteLastTask } from '../../reduxActions/calendarDataActions';
...
class TaskDeletePopup extends Component {
    ...
    handleConfirmation() {
        ...
        if ( this.props.lastFlag ) {
            store.dispatch( deleteLastTask( this.props.taskId, this.props.cellId, this.props.weekNum, this.props.year ) )
            .then(() => {
                Popup.close();
            })
        } else {
            store.dispatch( deleteTask( this.props.taskId, this.props.cellId, this.props.weekNum, this.props.year ) )
            .then(() => {
                Popup.close();
            })
      }
        ...
        return(
            <div className = 'popup'>
                ...
                <div className = 'popup__cta'>
                    <button className = 'popup__button' onClick = { this.handleConfirmation }>Tak</button>
                    <button className = 'popup__button' onClick = { this.handleCancelation }>Nie</button>
                </div>
            </div>
        )
    }
